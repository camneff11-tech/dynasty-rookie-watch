...
// The team's own totals for the game (box score "totals" row), for market-share math.
function extractTeamTotals(summary, teamName) {
  const teamBox = (summary?.boxscore?.players ?? []).find(
    (t) => t?.team?.displayName === teamName
  );
  if (!teamBox) return null;
  const lines = (teamBox.statistics ?? [])
...
const cachedPastWeek = unstable_cache(fetchWeekStrict, ["week-v3"], { revalidate: 60 * 60 * 6 });
const cachedLiveWeek = unstable_cache(fetchWeekStrict, ["week-v3-live"], { revalidate: 60 * 10 });
...
// Everything the rankings page needs: season games plus opponent RPI ranks.
export async function buildRankingInputs({ season, throughWeek, players, currentWeek }) {
  const [seasonData, rpi] = await Promise.all([

...
// One game: raw points, RPI-adjusted points, and the multiplier that was applied.
export function gamePerformance(w, config, scoring) {
  ...
  return {
    ...
    adj: fantasyPoints(adjust(w.totals, mult), scoring),
  };
}
...
export function scoreProspect(row, config, scoring) {
  ...
  const games = row.weeks.map((w) => gamePerformance(w, config, scoring));
